...
<div class="card">
  ...
  <form action="{{ url('kelas/' . $kelas->id) }}" method="POST" enctype="multipart/form-data" autocomplete="off">
    @csrf
    @method('PUT')
    <div class="card-body">
      <div class="mb-3">
      ...
      <div class="mb-3">
        ...
        <select class="form-select js-choice" id="pendidik_id" size="1" name="pendidik_id"
          ...
          <option value="{{ $pendidik->id }}" {{ old('pendidik_id', $kelas->pendidik_id)==$pendidik->id ? 'selected' : '' }}>{{
