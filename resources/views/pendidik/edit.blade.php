...
<div class="card">
  <div class="card-header">
    <h5>Tambah Pendidik</h5>
  </div>
  <form action="{{ url('pendidik/' . $pendidik->id) }}" method="POST" enctype="multipart/form-data" autocomplete="off">
    @csrf
    @method('put')
    <div class="card-body">
      <div class="mb-3">
        ...
        <input class="form-control @error('nama') is-invalid @enderror" id="nama" name="nama" type="text"
          ...
          placeholder="masukan nama lengkap" value="{{ old('nama', $pendidik->nama) }}" />
        @error('nama')
      ...
      <div class="mb-3">
        ...
        <input class="form-control @error('email') is-invalid @enderror" id="email" name="email" type="text"
          placeholder="masukan email" value="{{ old('email', $pendidik->email) }}" />
        @error('email')
        <span class="invalid-feedback" role="alert">{{ $message }}</span>
        @enderror
      </div>
      ...
      <div class="mb-3">
        ...
        <textarea class="form-control @error('alamat') is-invalid @enderror" id="alamat" name="alamat"
          rows="3">{{ old('alamat', $pendidik->alamat) }}</textarea>
        @error('alamat')
        <span class="invalid-feedback" role="alert">{{ $message }}</span>
        @enderror
      </div>
